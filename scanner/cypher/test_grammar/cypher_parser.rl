...
%%{
    machine cypher_parser_entry;

    # We don't describe the full grammar here – that's done by the
    # recursive-descent functions.  This machine just funnels every
    # byte into a single "accept everything" pattern so Ragel emits
    # valid C, and we break out immediately to call parse_cypher().
