...
includegraphics = '\\includegraphics' brackets? braces ;
bibliography = '\\bibliography' braces ;
bibliographystyle = '\\bibliographystyle' braces ;
include_cmd = '\\include' braces ;
input_cmd = '\\input' braces ;
documentclass = '\\documentclass' brackets? braces ;
...
NP = '(NP' @{n=0;} (left_parens|right_parens|parens_body)* :> ')' when{!n};

word='\\'alpha{1,15};
context = word (any{0,40}-word);

# Natural language word: sequence of letters with optional hyphens/apostrophes
nl_word = (alpha+ ([\-''] alpha+)*) - (alpha{1});

# ---- Sentence boundary detection ----

# Period/!/? immediately followed by newline → sentence end
sent_end_nl = [.!?] ('\n' | '\r' '\n'?);

# Period/!/? followed by space + uppercase letter → new sentence  
sent_end_cap = [.!?] [ \t]+ [A-Z];

# Known abbreviations that absorb their period (no sentence break)
abbrev = 'e.g' | 'i.e' | 'cf' | 'vs' | 'w.r.t' | 'et al'
       | 'Eq' | 'Ref' | 'Fig' | 'Sec' | 'Vol' | 'No' | 'pp'
       | 'Dr' | 'Mr' | 'Mrs' | 'Ms' | 'Prof' | 'Sen'
       | 'eds' | 'approx' | 'esp' | 'inc' | 'est';

# Abbreviation with trailing period: e.g. → keep as one token
abbrev_dot = abbrev '.';

# Single uppercase letter + period: J. K. C. (initials)
initial = [A-Z] '.';

# Prose word: letters, hyphens, apostrophes
prose = alpha+ ([\-'] alpha+)*;
}%%
