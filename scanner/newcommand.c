...
static int skip_this_macro = 0;

static int is_internal_name(const char *name) {
  static const char *sys_names[] = {
    "section", "subsection", "subsubsection", "paragraph", "subparagraph",
    "thebibliography", "theequation", "thefigure", "thetable",
    "newblock", "cite", "nocite", "bibitem", "bibliography",
    "label", "ref", "pageref", "begin", "end", "item",
    "appendix", "tableofcontents", "listoffigures", "listoftables",
    "title", "author", "date", "maketitle", "thanks",
    "baselinestretch", "newcommand", "renewcommand", "providecommand",
    "def", "edef", "xdef", "gdef", "let",
    "footnote", "footnotemark", "footnotetext",
    "caption", "part", "chapter", "table", "figure",
    "hspace", "vspace", "hfill", "vfill", "hfil",
    "textrm", "textit", "textsf", "texttt", "textbf",
    "emph", "operatorname", "ensuremath", "text",
    NULL
  };
  int i;
...
void learn_body(void) {
  ...
  *ep = '\0';
  ...
  if (!skip_this_macro && body_len < MAXMACROBODYLEN - 1) {
    int *p;
    for (p = expansion; *p != 0; p++)
      if (*p == '@') { skip_this_macro = 1; break; }
  }

  if (!skip_this_macro && body_len < MAXMACROBODYLEN - 1) {
    intcpy(body[NEXTFREEMACRO], expansion);
    NEXTFREEMACRO = NEXTFREEMACRO + 1;
  } else {
    macro[NEXTFREEMACRO][0] = '\0';
  }
  skip_this_macro = 0;
}
...
void learn_keyword(void) {
  ...
  *cp = '\0';
  ...
  skip_this_macro = is_internal_name(name);
